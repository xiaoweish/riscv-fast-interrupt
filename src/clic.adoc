...
[%autowidth,%hardbreaks,options="header"]
|===
|Date        |   	Description
...
[%autowidth,%hardbreaks,frame=all,grid=all,options="header"]
|===
...
[%autowidth,%hardbreaks,options="header"]
|===
| miselect{nbsp}  |   mireg  | mireg register state    |   mireg2  | mireg2 register state    |    description
|           |   bits   |                         |   bits    |                          |
...
sscratchcsw, sscratchcswl) by S-mode or a lower privilege level
...
edge-triggered interrupts and clearing `clicintip[__i__]` via indirect CSR access while same privilege level mstatus.mie is enabled requires mireg register state to be part of the interrupt handler's overall context state save/restore.
